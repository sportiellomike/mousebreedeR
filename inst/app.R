...
# print(df)
  ...
  output$summarize_potential_pup_output <-DT::renderDT({
    ...
    summarize_potential_pup_output$Mom<-vapply(strsplit(summarize_potential_pup_output$Mom_x_Dad, "x", fixed = F), "[", "", 1)
    summarize_potential_pup_output$Dad<-vapply(strsplit(summarize_potential_pup_output$Mom_x_Dad, "x", fixed = F), "[", "", 2)
